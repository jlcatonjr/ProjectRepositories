...
def dict_of_figs_to_dropdown_fig(figs, show_fig=True):
    ...
    for key in keys:
        ...
        for i in range(current_num_rows + 1, num_rows + 1):
            for prefix in ['yaxis', 'xaxis', 'annotations']:
                if f'{prefix}{i}' in layout_json:
                    reset_layout[f'{prefix}{i}'] = go.layout.XAxis(visible=False).to_plotly_json() if prefix.startswith('x') else go.layout.YAxis(visible=False).to_plotly_json()
                    del layout_json[f'{prefix}{i}']
            for j, annotation in enumerate(layout_json.get('annotations', [])):
                if annotation.get('xref', '') == f'x{i}':
                    reset_layout[f'annotations[{j}]'] = {'visible': False}
                    del layout_json[f'annotations[{j}]']                    
           
        final_layout = {**layout_json, **reset_layout}
...
def dict_of_figs_to_dropdown_fig(figs, show_fig = True):
    ...
    for key, fig in figs.items():
        ...
        for i in range(current_num_rows + 1, num_rows + 1):
            for attr in ['yaxis', 'xaxis']:
                reset_layout[f'{attr}{i}'] = {}
            for j in range(len(layout_json.get('annotations', []))):
                reset_layout[f'annotations[{j}]'] = {}
                reset_layout[f'annotations[{j}]'] = {'visible': False}
    ...
    dropdown_buttons = [
        {
            ...
            "args": [
                {"visible": show_traces[key]},
                final_layout#**figs[key].layout.to_plotly_json()}
            ]
        } for key in keys
    ]
    ...
    #             if f'{key}{i}' in layout_json:
    #                 reset_layout[f'{key}{i}'] = {'visible': False}

    #                 del layout_json[f'{key}{i}']
    #         for j, annotation in enumerate(layout_json.get('annotations', [])):
    #             if f'annotations[{j}]' in layout_json and annotation.get('xref', '') == f'x{i}':
    #                 reset_layout[f'annotations[{j}]'] = {'visible': False}
    #                 del layout_json[f'annotations[{j}]']                    
